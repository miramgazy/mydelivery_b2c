...
onMounted(async () => {
    if (authStore.user) {
        ...
        if (defaultAddr) {
            ...
            // Если адресов нет — предлагаем самовывоз по умолчанию
            switchToPickup()
        }
    }
...
const submitOrder = async () => {
    // Проверка наличия телефона в профиле. Если нет — отправляем на онбординг номера
    if (!authStore.user?.phone) {
        telegramService.showAlert('Для оформления заказа необходимо указать номер телефона. Пожалуйста, поделитесь своим контактом, чтобы мы могли связаться с вами.')
        router.push({ path: '/onboarding/phone', query: { return: '/checkout' } })
        return
    }
