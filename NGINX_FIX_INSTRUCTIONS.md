...
1. ❌ Первый server блок без `listen` и `server_name` - не будет работать
2. ❌ HTTPS блок имеет `return 404` вместо проксирования
3. ❌ Нет обработки Punycode домена (`xn--b2-delivery-toj.mevent.kz`)
4. ❌ Порт 3008 - это для другого проекта

## Решение:

### Шаг 1: Обновить конфигурацию

Замените содержимое файла `/etc/nginx/sites-available/b2c-delivery.mevent.kz` на:
...
2. ✅ HTTPS блок теперь проксирует запросы вместо `return 404`
3. ✅ Добавлена обработка Punycode домена
4. ✅ Оба варианта домена принимаются и редиректятся на нормальный
